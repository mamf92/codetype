...
export default function Settings() {
  ...
  const onClearHistory = (): void => {
    if (!window.confirm('Clear every recorded drill? This cannot be undone.')) return
    clearHistory()
  }
      ...
      <div className="reveal flex flex-col gap-3.5" style={{ animationDelay: '0.12s' }}>
        ...
        <Panel accent="fault" className="flex flex-wrap items-center justify-between gap-4">
          <p className="max-w-lg text-[11px] leading-relaxed text-muted">
            Progress lives only in this browser's local storage — {progress.sessions.length}{' '}
            recorded {progress.sessions.length === 1 ? 'drill' : 'drills'} so far. Clearing it is
